...
Adapted from carls-app/map-tiles. Renamed from verify.py because this repo
already has one: scripts/verify.py checks the *data*, this checks the *tiles*.
...
def main(dist: str) -> None:
    ...
    for name in ("style.json", "style-pmtiles.json"):
        ...
        for unused in sorted(CAMPUS_LAYERS - referenced):
            problems.append(f"{name}: never draws campus layer {unused!r}")
